defmodule PancreaRSS.PollTest do
  use ExUnit.Case

  test "start_link/1 validates uri without scheme" do
    assert {:error, :invalid_url} =
             PancreaRSS.Poll.start_link(url: "example.test/feed.rss", name: :test)
  end

  test "start_link/1 validates name" do
    assert {:error, :invalid_name} =
             PancreaRSS.Poll.start_link(url: "https://example.test/feed.rss", name: "invalid")
  end

  test "start_link/1 starts a new process" do
    assert {:ok, pid} =
             PancreaRSS.Poll.start_link(url: "https://example.test/feed.rss", name: :rss_example)
